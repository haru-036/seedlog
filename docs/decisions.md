...
## ADR-010: 通知トリガーは push を維持し、通知先は PR 作者優先にする

**日付:** 2026-03-14
**状況:** main 直 push も取りこぼしたくない一方で、PR ワークフローではマージ実行者ではなく PR 作者に通知したい要件が出た。

**決定:** 通知トリガーは `push`（main のみ）を維持し、push 受信後に commit から関連 PR を解決して、解決できた場合は PR 作者を通知対象にする。PR が解決できない場合は `pusher.name` にフォールバックする。

**理由:**

- 直 push と PR merge の両方を単一トリガーで扱え、運用がシンプル
- `pull_request` イベントを併用しないため、重複通知制御を実装しなくて済む
- 既存の main push フィルタ（ADR-008）を保ったまま、通知先の精度だけ改善できる

**却下した選択肢:**

- `pull_request` イベントを追加して通知する → main push と二重発火し、重複制御が必要になる
- 常に `pusher.name` に通知する → PR を別ユーザーがマージしたときに、実作業者へ通知できない
